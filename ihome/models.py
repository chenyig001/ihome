...
class User(BaseModel, db.Model):
    ...
    # def generate_password(self, origin_password):
    #     '''对密码加密'''
...
class House(BaseModel, db.Model):
    ...
    def to_full_dict(self):
        ...
        for image in self.images:
            img_list.append(constants.QINIU_URL_DOMAIN + image.url)
        ...
        for order in orders:
            comment = {
                ...
                "user_name": order.user.name if order.user.name != order.user.mobile else "匿名用户",  # 发表评论用户
...
class Order(BaseModel, db.Model):
    ...
    trade_no = db.Column(db.String(130))  # 交易的流水号
